...
impl SingleTopicConsumer {
    pub fn new(
        ...
        // SASL — overrides kafka_tls's security.protocol if both are set
        if let Some(ref protocol) = common_config.kafka_security_protocol {
            client_config.set("security.protocol", protocol);
        }
        if let Some(ref mechanism) = common_config.kafka_sasl_mechanism {
            client_config.set("sasl.mechanism", mechanism);
        }
        if let Some(ref username) = common_config.kafka_sasl_username {
            client_config.set("sasl.username", username);
        }
        if let Some(ref password) = common_config.kafka_sasl_password {
